...
class GABeamSolver(ABCSolver):
    ...
    def solve(self, instance: DTPInstance) -> DTPSolution:
        ...
        best_idx = np.argmax([f[1] if f is not None else float('-inf') for f in fitness_scores])
        best_solution = fitness_scores[best_idx][0] if fitness_scores[best_idx] is not None else None
        best_fitness = fitness_scores[best_idx][1] if fitness_scores[best_idx] is not None else float('-inf')
        ...
        for generation in range(self.n_generations):
            ...
            if self.elitism > 0:
                ...
                sorted_indices = sorted(range(len(fitness_scores)), 
                                      key=lambda i: fitness_scores[i][1] if fitness_scores[i] is not None else float('-inf'),
                                      reverse=True)
    ...
    def _evaluate_route(
        ...
        total_time = sum(instance.tiempos[route[i], route[i+1]] for i in range(len(route) - 1))
        ...
        for idx in range(n_stops - 1):
            ...
            for capital, cargo, compras_hist, ventas_hist in beam:
                ...
                successors = self._generate_successors(
                    instance, current_port, next_port, capital, cargo, 
                    compras_hist, ventas_hist, idx
                )
        ...
        if idx < len(compras_hist) - 2:
            ...
            for num_items in range(min(3, len(opportunities) + 1)):
                ...
                for i in range(num_items):
                    ...
                    max_by_capital = cap_temp / precio_compra if precio_compra > 0 else float('inf')
                    max_by_capacity = cap_temp_disponible / peso if peso > 0 else float('inf')
                ...
                final_capital2 = new_capital - compras.dot(instance.precios_venta[:, current_port]) - travel_cost
    ...
    def _tournament_selection(
        ...
        best_idx = max(tournament, key=lambda i: fitness_scores[i][1] if fitness_scores[i] is not None else float('-inf'))
