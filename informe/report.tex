...
\textbf{Descripción de instancias especiales:}
\begin{itemize}
    \item \textbf{KNAPSACK:} Dos puertos, muchas mercancías con diferentes ratios peso/ganancia. Diseñada para evaluar la optimización de compras.
    \item \textbf{TSP:} Seis puertos, una mercancía con oferta unitaria. Precios diseñados para que solo valga vender en Ámsterdam. Enfoca el problema en la optimización de rutas.
\end{itemize}
...
\begin{table}[h]
\centering
\caption{Resultados comparativos (tiempo en segundos)}
\begin{tabular}{|l|c|c|c|c|}
\hline
\textbf{Instancia} & \multicolumn{2}{c|}{\textbf{Brute Force}} & \multicolumn{2}{c|}{\textbf{Greedy}} \\
\cline{2-5}
& \textbf{Beneficio} & \textbf{Tiempo} & \textbf{Beneficio} & \textbf{Tiempo} \\
\hline
TINY & Óptimo & $\sim$1-5s & Aprox. & $<$1ms \\
KNAPSACK & Óptimo & $\sim$0.5s & Aprox. 95\% & $<$1ms \\
TSP & --- & Inviable & Heurístico & $<$1ms \\
SMALL+ & --- & Inviable & Heurístico & $<$10ms \\
\hline
\end{tabular}
\end{table}
...
\textbf{Observaciones:}
\begin{enumerate}
    \item El algoritmo de fuerza bruta es viable solo para instancias muy pequeñas ($n \leq 3$, $m \leq 2$)
    \item El algoritmo greedy provee soluciones de calidad razonable (90-95\% del óptimo en instancias pequeñas) en tiempo despreciable
    \item Para instancias realistas ($n \geq 6$), el greedy es la única opción práctica
    \item La calidad de las soluciones greedy depende fuertemente de la estructura de precios
\end{enumerate}
